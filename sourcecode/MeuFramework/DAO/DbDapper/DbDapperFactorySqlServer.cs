...
namespace MeuFramework.DAO.DbDapper
{
    public class DbDapperFactorySqlServer : IDbDapper
    {
        ...
        public DbDapperCommand GetCommand(string sql)
        {
            return new DbDapperCommand() { Objeto = null, Sql = sql };
        }
        ...
        public ObservableCollection<T> Query<T>(DbDapperCommand comando)
        {
            using (var conexao = new SqlConnection(connectionString))
            {
                if (comando.Objeto == null)
                    return new ObservableCollection<T>(conexao.Query<T>(comando.Sql));
                else
        ...
        public T QuerySingle<T>(DbDapperCommand comando)
        {
            ...
            {
                if (comando.Objeto == null)
                    return conexao.Query<T>(comando.Sql).FirstOrDefault();
                else
